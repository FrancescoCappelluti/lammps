...
FixFRespAIDsf::FixFRespAIDsf(LAMMPS *lmp, int narg, char **arg) :
...
void FixFRespAIDsf::q_update_Efield_bond()
{
  ...
  double grij, expm2, fsp, ssp, tsp, ftsp;
  double ddamping[3];
  double fvp[3], svp[3], tvp[3], bondrvmprod, damping, factor_coul;
  ...
  for (bond = 0; bond < nbond_old; bond++) {
	...
	center_t = types[global_center - 1];
        ...
        pref -= f_shift;
        pref *= factor_coul * q_gen * rvminv * bondvinvsq;
        //Now pref is A * q_gen / (|rvm||rb|**2)
        fsp = 3.0 * bondrvmprod * rvminvsq;
        tsp = factor_coul * q_gen * bondvinvsq * rvminvsq * bondrvmprod *
          (f_shift * rvminv + expm2 * tgecuospi);
        //E^fic * rb / |rb|**3
        ftsp = pref * bondrvmprod * bondvinvsq;

        if (rvml < cutoff2 && dampflag > 0) {
          MathExtra::copy3(rvm, ddamping);
          damping = Efield_damping(rvml, ddamping, center_t, atom1_t, atom2_t);
          //tsp and ftsp are multiplied times damping. Because pref too will
        ...
        dEr_vals[bond][atom1_pos][0] += pref * fvp[0] - svp[0] - tvp[0];
        dEr_vals[bond][atom1_pos][1] += pref * fvp[1] - svp[1] - tvp[1];
        dEr_vals[bond][atom1_pos][2] += pref * fvp[2] - svp[2] - tvp[2];
        ...
        dEr_vals[bond][atom2_pos][0] += pref * fvp[0] - svp[0] + tvp[0];
        dEr_vals[bond][atom2_pos][1] += pref * fvp[1] - svp[1] + tvp[1];
        dEr_vals[bond][atom2_pos][2] += pref * fvp[2] - svp[2] + tvp[2];
      }
